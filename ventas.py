...
def render_modulo_ventas():
    ...
    with t1:
        if not lista_productos:
            ...
            if st.button("➕ Agregar al Carrito POS"):
                ...
                # ALERTA PREMIUM AL AGREGAR PRODUCTO
                st.toast(f"⚜️ **CARRITO ACTUALIZADO** | Se agregó: {cant_v}x {nombre_p} correctamente.", icon="✅")
                        ...
                        if st.button("🚀 CERRAR VENTA Y GUARDAR COMPROBANTE"):
                            if confirmar_accion:
                                ...
                                if exito:
                                    ...
                                    # ALERTA PREMIUM AL CERRAR VENTA EXITOSAMENTE
                                    st.toast(f"⚜️ **VENTA REGISTRADA** | Factura generada para {st.session_state.pos_cliente} por ${seguro_int(total_final):,.0f} COP.", icon="✅")
                ...
                with col_accion2:
                    if st.button("🧹 Descartar / Vaciar Carrito"):
                        if confirmar_accion:
                            st.session_state.carrito = []
                            st.session_state.pos_cliente = "Consumidor Final"
                            # ALERTA PREMIUM AL VACIAR EL CARRITO
                            st.toast("⚠️ **CARRITO VACIADO** | Se han removido todos los artículos seleccionados.", icon="🗑️")
                            st.rerun()
                        else:
    ...
    with t2:
        ...
        if not df_ventas.empty:
            ...
            # --- NUEVA SECCIÓN DE ANULACIÓN Y REVERSIÓN DINÁMICA ---
            st.markdown("### 🛠️ Panel de Gestión y Control Fiscal")
            confirmar_borrado_v = st.checkbox("⚠️ AUTORIZACIÓN CRÍTICA: Deseo anular esta factura por completo y regresar el stock al almacén.")
            
            if st.button("🗑️ Anular Factura Seleccionada"):
                if confirmar_borrado_v:
                    conn = conectar_db()
                    cursor = conn.cursor()
                    try:
                        # 1. Extraer los detalles del texto para reversar cantidades en el inventario
                        cursor.execute("SELECT productos_detalles FROM ventas WHERE id = ?", (id_sel,))
                        detalles_texto = cursor.fetchone()[0]
                        
                        # Analizar las líneas del tiquete guardado
                        for linea in detalles_texto.split("\n"):
                            linea = linea.strip()
                            if not linea or linea.startswith("CANT") or linea.startswith("-"):
                                continue
                            try:
                                partes = linea.split()
                                if len(partes) >= 2:
                                    indigo_devuelta = int(partes[0])
                                    # El nombre del producto puede contener espacios, lo buscamos en el inventario
                                    # Para ser ultra seguros y precisos sin romper nada, buscamos por coincidencia de texto inicial
                                    nombre_parcial = partes[1][:15] 
                                    
                                    cursor.execute("UPDATE inventario SET cantidad = cantidad + ? WHERE nombre LIKE ?", (indigo_devuelta, f"{nombre_parcial}%"))
                            except Exception:
                                pass # Ignorar líneas que no correspondan a ítems
                        
                        # 2. Borrar la venta de forma definitiva
                        cursor.execute("DELETE FROM ventas WHERE id = ?", (id_sel,))
                        conn.commit()
                        
                        # ALERTA PREMIUM AL ANULAR UNA FACTURA
                        st.toast(f"⚠️ **FACTURA ANULADA** | Comprobante #{id_sel} eliminado y stock devuelto al almacén.", icon="🗑️")
                        
                        st.success(f"Factura #{id_sel} anulada con éxito. El inventario ha sido devuelto.")
                        st.rerun()
                    except Exception as e:
                        conn.rollback()
                        st.error(f"Error en la base de datos al anular: {str(e)}")
                    finally:
                        conn.close()
                else:
                    st.error("🚨 Debe activar la casilla de autorización crítica para eliminar la factura.")
            st.write("---")
