...
def render_modulo_ventas():
                        ...
                        if st.button("🚀 CERRAR VENTA Y GUARDAR COMPROBANTE"):
                            if confirmar_accion:
                                ...
                                if exito:
                                    ...
                                    # REPARACIÓN: Eliminado el parámetro 'key' que causaba TypeError
                                    st.markdown(f"""
    ...
    with t2:
        ...
        if not df_ventas.empty:
            ...
            # REPARACIÓN: Se encapsula en un st.container() dedicado para evitar el quiebre de JavaScript en el DOM (removeChild)
            with st.container():
                st.markdown(f"""
                <div class="factura-premium">
                    <div class="factura-header">
