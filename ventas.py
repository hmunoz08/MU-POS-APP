...
def aplicar_estilo_luxury():
    st.markdown("""
        ...
        /* DISEÑO DE LA FACTURA HERMOSA (LUXURY GOLD & NOIR) */
        .factura-premium {
...
def procesar_factura_completa(cliente, metodo, carrito, subtotal, imp_total, desc_total, total_neto):
    ...
    try:
        for item in carrito:
            ...
            string_detalles += f"- {nombre_prod} [{cantidad}x${precio_v:,.0f}]: ${cantidad*precio_v:,.0f} COP\n"
...
def render_modulo_ventas():
    ...
    with t2:
        ...
        if not df_ventas.empty:
            ...
            # SECCIÓN CORREGIDA CON ST.MARKDOWN Y UNSAFE_ALLOW_HTML COMPLETO
            st.markdown(f"""
