st.markdown("""
    <style>
    /* Asegura que la factura sea 100% responsive */
    .factura-box {
        width: 100% !important;
    ...
    /* Ajusta los botones para que sean fáciles de tocar */
    .stButton>button {
...
def aplicar_estilo_luxury():
    st.markdown("""
        ...
        /* DISEÑO DE FACTURA LUXURY GOLD & NOIR */
        .factura-premium {
...
def inicializar_tabla_ventas():
    ...
    # SOLUCIÓN AL ERROR DE COLUMNA FALTANTE (MIGRACIÓN INTEGRAL):
    # Validamos si la columna 'cliente' existe, si no, agregamos de manera dinámica
    try:
        cursor.execute("SELECT cliente FROM ventas LIMIT 1")
    except sqlite3.OperationalError:
        # Si falla porque la columna o la tabla vieja no corresponden, saneamos la tabla por completo
...
def seguro_int(val):
    """Convierte de forma segura valores a entero evitando errores de bytes/caracteres raros (Imagen 4)"""
    try:
...
def procesar_factura_completa(cliente, metodo, carrito, subtotal, imp_total, desc_total, total_neto):
    ...
    try:
        for item in carrito:
            ...
            utilidad_item = (precio_v * cantidad) - (seguro_int(costo_u) * cantidad)
            total_utilidad += utilidad_item
            
            string_detalles += f"{nombre_prod:<25} {cantidad}x ${precio_v:,.0f}\n"
            cursor.execute("UPDATE inventario SET cantidad = ? WHERE id = ?", (stock_act - cantidad, id_p))
...
def render_modulo_ventas():
    ...
    st.title("🖥️ TERMINAL DE PUNTO DE VENTA (POS PREMIUM) 🖥️")
    st.write("---")
    
    t1, t2, t3 = st.tabs(["🛒 Facturación POS", "🖨️ Historial y Despacho", "📊 Cierre de Caja"])
    ...
    with t1:
        if not lista_productos:
            ...
            st.subheader("1. Cabecera de la Transacción")
            col_cl, col_me = st.columns(2)
            with col_cl: 
                st.session_state.pos_cliente = st.text_input("Cliente Recibidor:", value=st.session_state.pos_cliente).strip()
            with col_me: 
                metodo_pago = st.selectbox("Forma de Recaudo:", ["Efectivo", "Tarjeta de Débito", "Tarjeta de Crédito", "Transferencia Bancaria", "Nequi / Daviplata"])
                
            st.write("---")
            st.subheader("2. Selección Guiada de Ítems")
            
            opciones_prod = {p[0]: f"{p[1]} (Stock: {p[2]} | Costo: ${seguro_int(p[3]):,.0f})" for p in lista_productos}
            prod_seleccionado = st.selectbox("Elija el artículo a facturar:", options=list(opciones_prod.keys()), format_func=lambda x: opciones_prod[x])
            
            sku, nombre_p, stock_p, costo_p = [p for p in lista_productos if p[0] == prod_seleccionado][0]
            
            col_ca, col_pr = st.columns(2)
            with col_ca: cant_v = st.number_input("Unidades a vender:", min_value=1, max_value=int(stock_p), step=1, value=1)
            with col_pr: precio_v = st.number_input("Precio Venta Comercial ($):", min_value=0, step=500, value=seguro_int(costo_p * 1.3))
                    ...
                    if pago_recibido >= total_final:
                        if st.button("🚀 CERRAR VENTA Y GUARDAR COMPROBANTE"):
                            if confirmar_accion:
                                ...
                                st.error("🚨 Debes marcar la casilla de confirmación obligatoria para procesar la venta.")
                with col_accion2:
        ...
        if not df_ventas.empty:
            ...
            <div class="factura-premium">
                ...
                <div class="factura-totales">
                    Subtotal Bruto: ${seguro_int(factura_act['subtotal']):,.0f} COP<br>
    ...
    with t3:
        ...
        st.write("#### 💳 Validación Cruzada por Medios de Pago")
        if medios_data:
