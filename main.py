...
def realizar_backup_local():
    # REPARACIÓN EXECUTABLE: Forzar ruta absoluta al directorio base para no perder la BD
    base_dir = os.path.abspath(".")
    origen = os.path.join(base_dir, 'syscafecopia.db')
    backups_dir = os.path.join(base_dir, 'backups_seguridad')
    
    if not os.path.exists(backups_dir):
        # REPARACIÓN: exist_ok=True evita que la app colapse en Streamlit Cloud o PC si la carpeta ya existe
        os.makedirs(backups_dir, exist_ok=True)
        
    if os.path.exists(origen):
        fecha_hoy = datetime.now().strftime("%Y-%m-%d")
        destino = os.path.join(backups_dir, f'backup_{fecha_hoy}.db')
        if not os.path.exists(destino):
            shutil.copy2(origen, destino)
            # ALERTA DE RESPALDO REALIZADO EXPENDIDO CON ÉXITO
            st.toast("💾 **RESPALDO GENERADO** | Copia espejo guardada en el almacenamiento local.", icon="⚜️")
...
# Bandera interna para evitar que el brindis de bienvenida se duplique al recargar páginas internas
if 'toast_bienvenida_mostrado' not in st.session_state:
    st.session_state.toast_bienvenida_mostrado = False
# Rastreador de módulo previo para confirmar el cambio de entorno operativo al usuario
if 'modulo_previo' not in st.session_state:
    st.session_state.modulo_previo = None
...
if mostrar_pantalla_login():
    ...
    # ALERTA DE CONFIRMACIÓN DE INICIO DE SESIÓN
    if not st.session_state.toast_bienvenida_mostrado:
        st.toast(f"⚜️ **ACCESO AUTORIZADO** | Bienvenido {usuario.upper()} ({rol}).", icon="✅")
        st.session_state.toast_bienvenida_mostrado = True
    ...
    # ALERTA DE CONFIRMACIÓN CAMBIO DE ENTORNO OPERATIVO
    if st.session_state.modulo_previo != modulo_maestro:
        st.toast(f"🔄 **ENTORNO CARGADO** | {modulo_maestro.split('. ')[1]}", icon="⚜️")
        st.session_state.modulo_previo = modulo_maestro
    
    if st.sidebar.button("🔒 Cerrar Sesión Segura", use_container_width=True):
        st.toast("⚠️ **SESIÓN FINALIZADA** | Conexión cerrada de forma segura.", icon="🔒")
        st.session_state.autenticado = False
        st.session_state.usuario_actual = None
        st.session_state.rol_actual = None
        st.session_state.toast_bienvenida_mostrado = False
