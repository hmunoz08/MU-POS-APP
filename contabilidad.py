...
def render_modulo_contabilidad():
    ...
    with tab2:
        with st.form("form_gastos", clear_on_submit=True):
            ...
            if st.checkbox("Certifico la salida conforme de caja para este egreso.") and st.form_submit_button("Asentar Egreso en Libro"):
                if concepto:
                    registrar_gasto_general(concepto, categoria, monto_g)
                    st.success("Asiento de egreso incorporado al balance diario.")
                    st.rerun()
                else: st.error("Por favor, especifique el concepto del egreso.")