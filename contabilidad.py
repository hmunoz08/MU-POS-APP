...
def ejecutar_respaldo_seguridad():
    ...
    if os.path.exists('syscafecopia.db'):
        # REPARACIÓN STREAMLIT CLOUD: exist_ok=True evita el colapso si la carpeta ya existe
...
def render_modulo_contabilidad():
    ...
    # ADICIÓN: Encabezado principal estandarizado con los otros módulos
    ...
    with tab_dashboard:
        ...
        if total_ingresos_vivos > 0:
            ...
            if porcentaje_gasto < 40:
                ...
                st.warning("⚠️ ALERTA DE SEGUIMIENTO: Flujo operativo balanceado. Vigilar el crecimiento de pasivos and jornales.")
            else:
